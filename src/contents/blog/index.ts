...
export const Posts: PostToken[] = Object.keys(NamedPosts)
  .filter((key: string): key is keyof typeof NamedPosts => Object.prototype.hasOwnProperty.call(NamedPosts, key))
  .map((key) => NamedPosts[key])
  .filter((post): post is PostToken => Post.is(post));

export const OrderedPosts: PostToken[] = Posts.sort((a: PostToken, b: PostToken) => {
